defmodule ProtocolPatched do
  @doc """
  Defines a protocol.

  See the `ProtocolPatched` module for more information.
  """
  defmacro defprotocol(name, do_block)

  defmacro defprotocol(name, do: block) do
    ProtocolPatched.__protocol__(name, do: block)
  end
  ...
  Explicit derivations can now be called via `__deriving__/3`:
      ...
      require ProtocolPatched
      ProtocolPatched.derive(Derivable, ImplStruct, :oops)
      Derivable.ok(%ImplStruct{a: 1, b: 1})
  ...
  defmacro derive(protocol, module, options \\ []) do
    quote do
      ProtocolPatched.__derive__(
        [{unquote(protocol), unquote(options)}],
        unquote(module),
        __ENV__
      )
    end
  ...
  ## Examples
      ...
      iex> mods = ProtocolPatched.extract_protocols([path])
  ...
  ## Examples
      ...
      # Get Elixir's ebin directory path and retrieve all protocols
      iex> path = :code.lib_dir(:elixir, :ebin)
      iex> mods = ProtocolPatched.extract_impls(Enumerable, [path])
      iex> List in mods
      true
  ...
  attribute:
      ...
      ProtocolPatched.consolidated?(Enumerable)
  ...
  def __protocol__(name, do: block) do
    quote do
      defmodule unquote(name) do
        ...
        import ProtocolPatched, only: [def: 1]
  ...
  defp after_defprotocol do
    quote bind_quoted: [built_in: __built_in__()] do
      ...
      @spec __protocol__(:functions) :: unquote(ProtocolPatched.__functions_spec__(@functions))
  ...
  defp do_defimpl(protocol, do: block, for: for) do
    ...
    quote do
      ...
      ProtocolPatched.assert_protocol!(protocol)
      ProtocolPatched.__ensure_defimpl__(protocol, for, __ENV__)
  ...
  def __ensure_defimpl__(protocol, for, env) do
    if ProtocolPatched.consolidated?(protocol) do
      message =
        "the #{inspect(protocol)} protocol has already been consolidated, an " <>
          ...
          "\"Consolidation\" section in the ProtocolPatched module documentation"
